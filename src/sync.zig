...
pub const server = struct { // MARK: server
	...
	pub fn executeUserCommand(source: *main.server.User, reader: *BinaryReader) !void {
		...
		// --- ASHFRAME CUSTOM (Container Open Lock Intercept) ---
		if (typ == .open) {
			if (payload.open.source == .blockInventory) {
				const target_pos = payload.open.source.blockInventory;
				// CHANGED: Use the top-level 'main.server.storage' package path alias directly
				if (main.server.storage.chest_locks.get(target_pos)) |lock| {
					if (lock.lock_type == 1) {
						const player_key = source.newKeyString orelse source.name;
						if (!std.mem.eql(u8, player_key, lock.owner_key)) {
							if (std.mem.indexOf(u8, lock.allowed_keys, player_key) == null) {
								source.sendMessage("#ff0000Access Denied: This chest is private property.", .{});
								main.network.protocols.inventory.sendFailure(source.conn);
								return; // Drops the open operation right here cleanly!
							}
						}
					}
				}
