...
var fsrPipeline: graphics.ComputePipeline = undefined;
var fsrOutputTexture: graphics.Texture = undefined; // Declared only once now!
var fsrUboID: c_uint = 0;
var fsrUboData: extern struct {
	Const0: [4]u32 = @splat(0),
	Const1: [4]u32 = @splat(0),
	Const2: [4]u32 = @splat(0),
	Const3: [4]u32 = @splat(0),
	Const0RCAS: [4]u32 = @splat(0),
	Extents: [4]u32 = @splat(0),
	Sample: [4]u32 = @splat(0),
} = .{};
...
pub fn init() void {
	...
	fsrPipeline = graphics.ComputePipeline.init(
		"assets/cubyz/shaders/fsr_easu.compute.base.glsl",
		"",
		null,
	);

	fsrOutputTexture = .init();
	c.glGenTextures(1, &fsrOutputTexture.textureID);
	c.glBindTexture(c.GL_TEXTURE_2D, fsrOutputTexture.textureID);
	c.glTexImage2D(c.GL_TEXTURE_2D, 0, c.GL_RGBA32F, @intCast(Window.width), @intCast(Window.height), 0, c.GL_RGBA, c.GL_FLOAT, null);
	c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_MIN_FILTER, c.GL_LINEAR);
	c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_MAG_FILTER, c.GL_LINEAR);
	c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_WRAP_S, c.GL_CLAMP_TO_EDGE);
	c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_WRAP_T, c.GL_CLAMP_TO_EDGE);

	c.glGenBuffers(1, &fsrUboID);
	c.glBindBuffer(c.GL_UNIFORM_BUFFER, fsrUboID);
	c.glBufferData(c.GL_UNIFORM_BUFFER, @sizeOf(@TypeOf(fsrUboData)), &fsrUboData, c.GL_DYNAMIC_DRAW);
	c.glBindBufferBase(c.GL_UNIFORM_BUFFER, 0, fsrUboID);
...
fn fsrSetupCon(
	...
	con2: *[4]u32,
	con3: *[4]u32,
	inputViewportWidthInPixels: f32,
	inputViewportHeightInPixels: f32,
	inputSizeWidthInPixels: f32,
	inputSizeHeightInPixels: f32,
	outputSizeWidthInPixels: f32,
	outputSizeHeightInPixels: f32,
) void {
	con0[0] = @bitCast(inputViewportWidthInPixels / outputSizeWidthInPixels);
	con0[1] = @bitCast(inputViewportHeightInPixels / outputSizeHeightInPixels);
	con0[2] = @bitCast((@as(f32, 0.5) * inputViewportWidthInPixels / outputSizeWidthInPixels) - @as(f32, 0.5));
	con0[3] = @bitCast((@as(f32, 0.5) * inputViewportHeightInPixels / outputSizeHeightInPixels) - @as(f32, 0.5));

	con1[0] = @bitCast(@as(f32, 1.0) / inputSizeWidthInPixels);
	con1[1] = @bitCast(@as(f32, 1.0) / inputSizeHeightInPixels);
	con1[2] = @bitCast(@as(f32, 1.0));
	con1[3] = @bitCast(@as(f32, 0.0));

	con2[0] = @bitCast(@as(f32, -1.0) / inputSizeWidthInPixels);
	con2[1] = @bitCast(@as(f32, -1.0) / inputSizeHeightInPixels);
	con2[2] = @bitCast(@as(f32, 1.0) / inputSizeWidthInPixels);
	con2[3] = @bitCast(@as(f32, 1.0) / inputSizeHeightInPixels);

	con3[0] = @bitCast(@as(f32, 0.0));
	con3[1] = @bitCast(@as(f32, 4.0) / inputSizeWidthInPixels);
	con3[2] = @bitCast(@as(f32, 0.0));
	con3[3] = @bitCast(@as(f32, 0.0));
}
...
pub fn updateViewport(width: u31, height: u31) void {
	const scale: f32 = switch (settings.fsrMode) {
		.off => 1.0,
		.ultra_performance => 0.33333,
		.performance => 0.5,
		.balanced => 0.59,
		.quality => 0.67,
	};

	lastWidth = @trunc(@as(f32, @floatFromInt(width)) * scale);
	lastHeight = @trunc(@as(f32, @floatFromInt(height)) * scale);
	...
	c.glBindTexture(c.GL_TEXTURE_2D, fsrOutputTexture.textureID);
	c.glTexImage2D(c.GL_TEXTURE_2D, 0, c.GL_RGBA32F, @intCast(width), @intCast(height), 0, c.GL_RGBA, c.GL_FLOAT, null);
...
pub fn renderWorld(world: *World, ambientLight: Vec3f, skyColor: Vec3f, playerPos: Vec3d) void { // MARK: renderWorld()
	...
	// CHANGE lastWidth and lastHeight TO main.Window.width and main.Window.height HERE:
	const direction = crosshairDirection(game.camera.viewMatrix, lastFov, main.Window.width, main.Window.height);
	...
	var finalSourceTextureID = fsrOutputTexture.textureID;
	...
	if (settings.fsrMode != .off) {
		...
		fsrSetupCon(
			...
			&fsrUboData.Const2,
			&fsrUboData.Const3,
			@floatFromInt(lastWidth),
					@floatFromInt(lastHeight),
					@floatFromInt(lastWidth),
					@floatFromInt(lastHeight),
					@floatFromInt(main.Window.width),
					@floatFromInt(main.Window.height),
		);

		const targetSharpness: f32 = settings.fsrSharpening;

		fsrUboData.Const0RCAS = .{
			@bitCast(targetSharpness),
			0,
			0,
			0,
		};
		...
		fsrUboData.Extents = .{
			@intCast(lastWidth),
			@intCast(lastHeight),
			@intCast(main.Window.width),
			@intCast(main.Window.height),
		};

		c.glBindBuffer(c.GL_UNIFORM_BUFFER, fsrUboID);
		c.glBufferSubData(c.GL_UNIFORM_BUFFER, 0, @sizeOf(@TypeOf(fsrUboData)), &fsrUboData);
		c.glBindBufferBase(c.GL_UNIFORM_BUFFER, 0, fsrUboID);

		worldFrameBuffer.bindTexture(c.GL_TEXTURE1);
		c.glBindImageTexture(2, fsrOutputTexture.textureID, 0, c.GL_FALSE, 0, c.GL_WRITE_ONLY, c.GL_RGBA32F);

		const workGroupsX = (main.Window.width + 15) / 16;
		const workGroupsY = (main.Window.height + 15) / 16;
		c.glDispatchCompute(workGroupsX, workGroupsY, 1);

		c.glMemoryBarrier(c.GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
	} else {
		finalSourceTextureID = worldFrameBuffer.texture;
	}
	...
	c.glBindTexture(c.GL_TEXTURE_2D, finalSourceTextureID);
