...
// Global tracking variables for timing the frame rate
var lastTime: i64 = 0;
var frameCount: u32 = 0;
var currentFps: u32 = 0;
...
pub fn render() void {
	...
	// Calculate real-time FPS frame deltas
	const currentTime = main.timestamp().toMilliseconds();
	frameCount += 1;
	...
	// Evaluate configuration options to toggle text output visibility
	if (main.settings.showFps) {
		draw.print("FPS: {}", .{currentFps}, 0, 0, 12);
	}

	// Retain full profile reporting list layout if window status expands out of HUD mode
	if (!window.isHud) {
		var sum: isize = 0;
		var y: f32 = 24;
		...
		draw.print("Total GPU: {} µs", .{@divTrunc(sum, 1000)}, 0, 12, 8);
	}
}
