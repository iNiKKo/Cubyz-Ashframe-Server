...
const fsrModesText = [_][]const u8{ "Off", "Ultra Performance", "Performance", "Balanced", "Quality" };
...
fn fsrModeGetIndex() u16 {
	return @intFromEnum(settings.fsrMode);
}

fn fsrModeCallback(newValue: u16) void {
	settings.fsrMode = @enumFromInt(newValue);
	settings.save();
	main.Window.GLFWCallbacks.framebufferSize(null, main.Window.width, main.Window.height);
}

fn fsrSharpeningCallback(newValue: f32) void {
	settings.fsrSharpening = @round(newValue * 100.0) / 100.0;
	settings.save();
}

fn fsrSharpeningFormatter(allocator: main.heap.NeverFailingAllocator, value: f32) []const u8 {
	return std.fmt.allocPrint(allocator.allocator, "#ffffffFSR Sharpening: {d:.0}%", .{value * 100.0}) catch unreachable;
}

fn showFpsCallback(newValue: bool) void {
...
pub fn onOpen() void {
	...
	list.add(DiscreteSlider.init(.{0, 0}, 128, "#ffffffAnisotropic Filtering: ", "{}x", &anisotropy, switch (settings.anisotropicFiltering) {
		...
		list.add(DiscreteSlider.init(.{0, 0}, 128, "#ffffffAMD FSR 1.0 Upscaling: ", "{s}", &fsrModesText, fsrModeGetIndex(), &fsrModeCallback));
		list.add(ContinuousSlider.init(.{0, 0}, 128, 0.0, 1.0, settings.fsrSharpening, &fsrSharpeningCallback, &fsrSharpeningFormatter));
		list.add(CheckBox.init(.{0, 0}, 128, "Show FPS Counter", settings.showFps, &showFpsCallback));
		list.finish(.center);
